...
def construct_dataframe(file_path, number):
    ...
    for i, resp in enumerate(response):
        ...
        if i == len(response) - 1:
            ext_map_dict = {f"external_similarity_{k}": list(resp["scores"][j]["prompt_attention_score"].keys())[k] for
                            k in range(number)}
            para_map_dict = {
...
def calculate_auc_pcc(df, ext_map_dict, para_map_dict, number):
    ...
    for k in range(number):
        ...
        auc_parameter_knowledge_difference_rename = [[a, para_map_dict[k]] for a, k in
                                                     auc_parameter_knowledge_difference]
...
if __name__ == "__main__":
    ...
    if args.model_name == "llama2-7b":
        if args.dataset == "ragtruth":
            data_path = "ReDEeP-ICLR/ReDeEP/log/test_llama2_7B/llama2_7B_response_chunk.json"
    ...
    auc_external_similarity, _, auc_parameter_knowledge_difference, _ = calculate_auc_pcc(
        df.iloc[:, :int(df.shape[1] * 0.5)], ext_map_dict, para_map_dict, number)

    if args.model_name == "llama2-7b":
        if args.dataset == "ragtruth":
            top_n, top_k, alpha, m = 3, 4, 0.6, 1
        elif args.dataset == "dolly":
            top_n, top_k, alpha, m = 7, 3, 1.6, 1

    elif args.model_name == "llama2-13b":
        if args.dataset == "ragtruth":
            top_n, top_k, alpha, m = 9, 3, 1.8, 1
        elif args.dataset == "dolly":
            top_n, top_k, alpha, m = 11, 3, 0.2, 1

    elif args.model_name == "llama3-8b":
        if args.dataset == "ragtruth":
            top_n, top_k, alpha, m = 2, 5, 1.2, 1
        elif args.dataset == "dolly":
            top_n, top_k, alpha, m = 1, 1, 0.1, 1
    else:
        print("model name error")
        exit(-1)

    auc_difference_normalized, person_difference_normalized = calculate_auc_pcc_32_32(df, top_n, top_k, alpha,
                                                                                      auc_external_similarity,
                                                                                      auc_parameter_knowledge_difference,
                                                                                      m)
    if args.model_name == "llama2-7b":
        save_path = "ReDEeP-ICLR/ReDeEP/log/test_llama2_7B/ReDeEP(chunk).json"
    elif args.model_name == "llama2-13b":
